...
if(true) {
    ...
    $config->setRules([
        ...
        'single_blank_line_before_namespace'    => false,   // Why in the... This goes against PHP standards!
        // New Things
    ...
    $path = '/tmp/acephpcsfixer/'.$worker.'-'.$instance.'/';

    if(!is_dir('/tmp/acephpcsfixer')) {
        mkdir('/tmp/acephpcsfixer');
    }
    if(!is_dir($path)) {
        mkdir($path);
    }
